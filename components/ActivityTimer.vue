...
function toggle() {
  ...
  if (!remaining.value) remaining.value = 300
...
function reset() { pause(); remaining.value = 300 }
onUnmounted(pause)
</script>
...
<template>
  <div class="activity-timer" @click.stop @keydown.stop>
    ...
    <div class="timer-controls">
      <button type="button" @click="toggle">{{ running ? 'Pause' : remaining === 300 ? 'Start 5 minutes' : remaining === 0 ? 'Start again' : 'Resume' }}</button>
...
<style scoped>
.activity-timer { padding: 28px; background: #fff0dc; border: 1px solid #e4b678; border-radius: 18px; text-align: center; }
.timer-label { color: #9a4700; font-size: 17px; font-weight: 750; letter-spacing: 2px; }
.timer-clock { color: #242424; font-size: 86px; font-weight: 750; line-height: 1.2; margin: 10px 0; font-variant-numeric: tabular-nums; letter-spacing: -3px; }
